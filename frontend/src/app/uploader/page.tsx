...
export default function UploaderPage() {
  ...
  const handleGenerate = async () => {
    ...
    if (!file && !text.trim()) { setError('Please upload a file or paste notes.'); return; }
    ...
    const apiUrl = getApiUrl('/process-notes'); // Use the helper
  ...
  if (loadingUser) { return <p className="text-center text-white pt-40">Loading...</p>; }
    ...
    <div className="min-h-screen bg-background text-white flex flex-col items-center justify-center pt-24 px-4">
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-2xl space-y-6">
          <input type="text" placeholder="Enter a title..." value={title} onChange={(e) => setTitle(e.target.value)} className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-purple-500" />
          <div className="border-2 border-dashed border-gray-700 rounded-lg p-8 text-center">
            <label htmlFor="file-upload" className="cursor-pointer">
              <div className="flex flex-col items-center text-gray-400">
                <UploadIcon />
                <p className="mt-2">{file ? file.name : 'Drag & drop a file or click to upload'}</p>
              </div>
              <input id="file-upload" type="file" className="hidden" accept=".pdf,.txt" onChange={handleFileChange} />
            </label>
          </div>
          <div className="text-center text-gray-500">OR</div>
          <textarea value={text} onChange={(e) => { setText(e.target.value); setFile(null); }} placeholder="Paste your notes directly here..." className="w-full h-48 bg-gray-800 border border-gray-700 rounded-lg p-4 text-gray-300 focus:ring-2 focus:ring-purple-500"></textarea>
          <button onClick={handleGenerate} disabled={isProcessing} className="w-full p-3 bg-purple-600 hover:bg-purple-700 rounded-lg text-white font-semibold transition-colors disabled:bg-gray-500">
